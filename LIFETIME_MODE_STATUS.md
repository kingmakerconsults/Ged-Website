...
1. **Load new data files:**
   ```javascript
   const lifeEventsLong = await fetch('/data/life_events_long.json').then(r => r.json());
   const retirementConfig = await fetch('/data/retirement.json').then(r => r.json());
   ```
...
3. **Main simulation loop (auto-advance):**
   ...
   function tick() {
     ...
     expenses.housing = getMonthlyHousingCost(player, costOfLivingCfg.basket, regionMult);
     expenses.loans = loanPayment;
     
     // Net cash flow
     player.cash += net.net - Object.values(expenses).reduce((a, b) => a + b, 0);
...
4. **Retirement income switching:**
   ...
   function switchToRetirement() {
     ...
     player.retirementIncome = initRetirementIncome(player, careerState, retirementConfig);
