...
async function main() {
  ...
  for (const filePath of jsonFiles) {
    try {
      const { updated, replacements, hasChanges } =
        await canonicalizeImagePaths(filePath);
  ...
  console.log('\nNext steps:');
  console.log(
    '1. Update frontend code to use /images/ instead of /frontend/Images/'
  );
  console.log(
    '2. Update backend server routes to serve /images/ from frontend/public/images/'
  );
