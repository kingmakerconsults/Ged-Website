...
const TARGETS = [
  {
    ...
    topics: new Set(['math_geom_formulas']),
    subject: 'Math',
  },
  {
    file: path.join(ROOT, 'public', 'quizzes', 'math.quizzes.part1.json'),
    topics: new Set(['math_quant_stats_probability']),
    subject: 'Math',
  },
  {
    file: path.join(
    ...
    topics: new Set(['ss_us_hist_revolution', 'ss_us_hist_divided_nation']),
...
function main() {
  ...
  console.log('Wave 9 complexity uplift complete (no tier overrides):');
