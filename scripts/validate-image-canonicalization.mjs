...
async function validateImageReferences() {
  ...
  try {
    const trackedDist = execSync('git ls-files frontend/dist/', {
      encoding: 'utf8',
    }).trim();
    if (trackedDist) {
  ...
  for (const jsonPath of jsonFiles) {
    if (existsSync(jsonPath)) {
      ...
      if (legacyMatches && legacyMatches.length > 0) {
        ...
        console.log(
          `   ⚠ ${relativePath}: ${legacyMatches.length} legacy path(s)`
        );
        foundLegacy = true;
      }
    }
  ...
  console.log(
    `${largeFiles.length > 0 ? '⚠' : '✓'} Large images (>50MB): ${
      largeFiles.length
    }`
  );
  console.log(
    `${foundLegacy ? '⚠' : '✓'} Legacy paths: ${foundLegacy ? 'FOUND' : 'None'}`
  );
  console.log('='.repeat(60));
