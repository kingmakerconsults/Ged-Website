...
const PREMADE_PATH = path.join(__dirname, '..', 'backend', 'data', 'premade-questions.js');
  ...
  Object.entries(ALL_QUIZZES).forEach(([subject, data]) => {
    ...
    Object.entries(data.categories).forEach(([catName, category]) => {
      ...
      category.topics.forEach(topic => {
        ...
        if (updated > 0) {
          console.log(`✅ ${topic.id}: Tagged ${updated}/${topic.questions.length} questions → [${tags.join(', ')}]`);
