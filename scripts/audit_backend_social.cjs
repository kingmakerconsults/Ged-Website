#!/usr/bin/env node
// Audit backend/data/quizzes/social-studies for missing questions and placeholders
...
function main() {
  ...
  if (issues.length) {
    console.log(`[SOCIAL STUDIES AUDIT] Issues found: ${issues.length}`);
    for (const line of issues) console.log('- ' + line);
    process.exitCode = 1;
  } else {
    console.log('[SOCIAL STUDIES AUDIT] No issues found in backend/data/quizzes/social-studies');
    process.exitCode = 0;
  }
}
