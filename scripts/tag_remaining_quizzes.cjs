...
const BACKEND_QUIZZES = path.join(__dirname, '..', 'backend', 'data', 'quizzes');
...
function tagQuiz(quizId, folder) {
  ...
  try {
    ...
    const modifiedQuestions = questions.map(q => {
      ...
      if (q.challenge_tags && Array.isArray(q.challenge_tags) && q.challenge_tags.length > 0) {
        return q;
      }
      updated++;
    ...
    const newContent = `${header}module.exports = ${JSON.stringify(modifiedQuestions, null, 2)};\n`;
    
    fs.writeFileSync(filepath, newContent, 'utf8');
    
    console.log(`✅ ${quizId}: Tagged ${updated}/${questions.length} questions → [${tags.join(', ')}]`);
...
function main() {
  ...
  // Science quizzes
  console.log('Science:');
  ['sci_life_science_basics', 'sci_ecosystems_environment', 'sci_chem_fundamentals', 'sci_scientific_numeracy_core'].forEach(id => {
