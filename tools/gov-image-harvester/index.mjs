...
const SUBJECT_CONFIG = {
  ...
  'Social Studies': {
    ...
    allowedDomains: new Set(['loc.gov', 'archives.gov', 'census.gov', 'bls.gov', 'nps.gov', 'data.gov']),
    sitemaps: [
...
function generateKeywords(image, pageInfo, dominantType) {
  const sourceWords = [image.caption, image.alt, pageInfo.title]
    .filter(Boolean)
    .join(' ')
    .toLowerCase();
  const extras = dominantType ? [dominantType] : [];
  ...
  if (unique.length === 0) {
    unique.push('education');
  }
  return unique;
}

function ensureLicense(pageInfo) {
  return {
    license: 'Public Domain (US Federal)',
    licenseNote: pageInfo.licenseNote || ''
  };
}
    ...
    try {
      ...
      for (const image of candidateImages) {
        ...
        try {
          ...
          const { buffer, width, height, dominantType } = await downloadAndNormalize(image.srcAbs);
          ...
          const keywords = generateKeywords(image, pageInfo, dominantType);
          const { license, licenseNote } = ensureLicense(pageInfo);
