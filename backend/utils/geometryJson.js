...
const DEFAULT_MAX_DECIMALS = parseInt(
  process.env.GEOMETRY_MAX_DECIMALS || '2',
  10
);
const SANITIZER_FEATURE_ENABLED =
  process.env.GEOMETRY_JSON_SANITIZER_ENABLED !== 'false';
...
const validateGeometryStructure = (obj) => {
  ...
  if (
    !ensureNumber(obj.answerIndex) ||
    Math.floor(obj.answerIndex) !== obj.answerIndex
  ) {
    errors.push('"answerIndex" must be an integer.');
  } else if (
    Array.isArray(obj.choices) &&
    (obj.answerIndex < 0 || obj.answerIndex >= obj.choices.length)
  ) {
  ...
  if (obj.choiceRationales) {
    if (
      !Array.isArray(obj.choiceRationales) ||
      obj.choiceRationales.some((item) => typeof item !== 'string')
    ) {
      errors.push(
        '"choiceRationales" must be an array of strings when provided.'
      );
  ...
  if (!spec || typeof spec !== 'object' || Array.isArray(spec)) {
    ...
    if (
      ...
      !SUPPORTED_SHAPES.includes(spec.shape)
    ) {
      errors.push(
        spec.shape
          ? `Shape '${spec.shape}' is not supported.`
          : 'Missing "geometrySpec.shape".'
      );
    }
    ...
    if (
      ...
      if (
        ...
        if (!Array.isArray(params.points) || params.points.length < 3) {
          errors.push(
            'Polygon-based shapes must include at least three labeled points.'
          );
        }
      }
      ...
      if (spec.shape === 'rectangle') {
        ...
        if (params.origin && !validatePoint(params.origin)) {
          errors.push(
            '"geometrySpec.params.origin" must include numeric x and y.'
          );
        }
      }
      ...
      if (spec.shape === 'circle') {
        if (!validatePoint(params.center)) {
          errors.push(
            '"geometrySpec.params.center" must include numeric x and y.'
          );
        }
      ...
      if (spec.shape === 'line_angle') {
        if (
          !validatePoint(params.vertex) ||
          !validatePoint(params.ray1) ||
          !validatePoint(params.ray2)
        ) {
          errors.push(
            'Line angle specifications require vertex, ray1, and ray2 points.'
          );
        }
      }
      ...
      if (spec.shape === 'trapezoid') {
        if (!Array.isArray(params.points) || params.points.length < 4) {
          errors.push('Trapezoid must include at least four points.');
        }
      }

      if (spec.shape === 'composite') {
        if (!Array.isArray(params.rects) || params.rects.length < 1) {
          errors.push(
            'Composite shape must include a non-empty "rects" array.'
          );
        } else {
          params.rects.forEach((r, idx) => {
            if (!r || typeof r !== 'object') {
              errors.push(`rects[${idx}] must be an object.`);
              return;
            }
            ['x', 'y', 'width', 'height'].forEach((k) => {
              if (!ensureNumber(r[k])) {
                errors.push(`rects[${idx}].${k} must be a number.`);
              }
            });
          });
        }
      ...
      if (spec.shape === 'parallel_transversal') {
        const checkLine = (name) => {
          const line = params[name];
          if (!line || typeof line !== 'object') {
            errors.push(`"geometrySpec.params.${name}" must be an object.`);
            return;
          }
          if (!validatePoint(line.start)) {
            errors.push(
              `"geometrySpec.params.${name}.start" must include numeric x and y.`
            );
          }
          if (!validatePoint(line.end)) {
            errors.push(
              `"geometrySpec.params.${name}.end" must include numeric x and y.`
            );
          }
        };
        checkLine('line1');
        checkLine('line2');
        checkLine('transversal');
      }

      if (spec.shape === 'cone_net') {
        requireNumber(params, 'radius');
        requireNumber(params, 'slantHeight');
      }

      if (spec.shape === 'pyramid_net') {
        requireNumber(params, 'baseLength');
        requireNumber(params, 'slantHeight');
      }
      ...
      if (
        Object.prototype.hasOwnProperty.call(params, 'radius') &&
        !ensureNumber(params.radius)
      ) {
      ...
      if (params.sideLabels) {
        if (!Array.isArray(params.sideLabels)) {
          errors.push(
            '"geometrySpec.params.sideLabels" must be an array when provided.'
          );
        } else {
          params.sideLabels.forEach((entry, index) => {
            ...
            if (!Array.isArray(entry.between) || entry.between.length !== 2) {
              errors.push(
                `sideLabels[${index}].between must be an array of two point labels.`
              );
            }
      ...
      if (params.rightAngle) {
        if (
          !params.rightAngle.vertex ||
          typeof params.rightAngle.vertex !== 'string'
        ) {
          errors.push(
            '"geometrySpec.params.rightAngle.vertex" must be a string label.'
          );
        }
        if (
          Object.prototype.hasOwnProperty.call(params.rightAngle, 'size') &&
          !ensureNumber(params.rightAngle.size)
        ) {
          errors.push(
            '"geometrySpec.params.rightAngle.size" must be numeric when provided.'
          );
        }
      }
      ...
      if (params.labels) {
        if (!Array.isArray(params.labels)) {
          errors.push(
            '"geometrySpec.params.labels" must be an array when provided.'
          );
        } else {
          params.labels.forEach((label, index) => {
            if (
              ...
              errors.push(
                `labels[${index}] must include text, x, and y values.`
              );
            }
          });
        }
      ...
      if (params.segments) {
        if (!Array.isArray(params.segments)) {
          errors.push(
            '"geometrySpec.params.segments" must be an array when provided.'
          );
        } else {
          params.segments.forEach((segment, index) => {
            ...
            if (!Array.isArray(segment.from) || !Array.isArray(segment.to)) {
              errors.push(
                `segments[${index}] must provide numeric coordinate arrays for from/to.`
              );
            } else if (
              segment.from.length !== 2 ||
              segment.to.length !== 2 ||
              !segment.from.every(ensureNumber) ||
              !segment.to.every(ensureNumber)
            ) {
              errors.push(
                `segments[${index}] from/to must be [x,y] numeric coordinates.`
              );
            }
          });
        }
    ...
    if (spec.view && typeof spec.view === 'object') {
      ...
      ['xMin', 'xMax', 'yMin', 'yMax', 'padding', 'width', 'height'].forEach(
        (key) => {
          if (
            Object.prototype.hasOwnProperty.call(view, key) &&
            !ensureNumber(view[key])
          ) {
            errors.push(
              `"geometrySpec.view.${key}" must be numeric when provided.`
            );
          }
        }
      );
