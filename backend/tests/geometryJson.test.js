...
describe('geometry JSON sanitization and parsing', () => {
  ...
  test('normalizes segments from point objects to [x,y] arrays', () => {
    const raw = JSON.stringify({
      question: 'Q',
      choices: ['A', 'B', 'C', 'D'],
      answerIndex: 0,
      geometrySpec: {
        ...
        params: {
          ...
          segments: [{ from: { x: 10, y: 80 }, to: { x: 80, y: 80 } }],
        },
      },
    });

    const result = parseGeometryJson(raw, { onStage: () => {} });
    const seg = result.value.geometrySpec.params.segments[0];
    assert.deepStrictEqual(seg.from, [10, 80]);
    assert.deepStrictEqual(seg.to, [80, 80]);
  });

  test('normalizes sideLabels from/to into between format', () => {
    const raw = JSON.stringify({
      question: 'Q',
      choices: ['A', 'B', 'C', 'D'],
      answerIndex: 0,
      geometrySpec: {
        ...
        params: {
          ...
          sideLabels: [{ from: 'A', to: 'B', label: '12 cm' }],
        },
      },
    });

    const result = parseGeometryJson(raw, { onStage: () => {} });
    const sl = result.value.geometrySpec.params.sideLabels[0];
    assert.deepStrictEqual(sl.between, ['A', 'B']);
    assert.strictEqual(sl.text, '12 cm');
  });
});
