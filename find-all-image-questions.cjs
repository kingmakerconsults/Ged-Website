...
['Science', 'Math', 'RLA', 'Social Studies', 'Workforce Readiness'].forEach(
  (subj) => {
    const p = path.join(imagesDir, subj);
    if (fs.existsSync(p)) {
      availableImages[subj] = fs
        .readdirSync(p)
        ...
        quiz.questions.forEach((q, qIdx) => {
          ...
          if (imgUrl && imgUrl.trim()) {
            ...
            allQuestionsWithImages.push({
              ...
              imageName: path.basename(imgUrl),
              exists: exists,
              questionText: (q.question || q.questionText || '').substring(
                0,
                100
              ),
            });
...
fs.writeFileSync(
  'image-questions-detailed.json',
  JSON.stringify(allQuestionsWithImages, null, 2)
);
