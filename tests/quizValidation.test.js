...
function enumerateQuestionsFromDataTag({ filePath, data }) {
  ...
  if (Array.isArray(data)) {
    ...
    for (const quiz of data) {
      ...
      qArr.forEach((q, i) => items.push({ filePath, quizId: quiz.id || quiz.title || 'unknown', question: q, index: i + 1, style: 'txt' }));
    }
    return items;
  }
      ...
      for (const topic of topics) {
        ...
        if (Array.isArray(topic.quizzes)) {
          for (const quiz of topic.quizzes) {
            const qArr = Array.isArray(quiz.questions) ? quiz.questions : [];
            qArr.forEach((q, i) => items.push({ filePath, quizId: quiz.quizId || quiz.id || `${topic.id || topic.title || 'topic'}`, question: q, index: i + 1, style: 'mc' }));
          }
        }
        if (Array.isArray(topic.questions)) {
          const quizId = topic.id || topic.title || `${subjectName}:${catName}`;
          topic.questions.forEach((q, i) => items.push({ filePath, quizId, question: q, index: i + 1, style: 'mc' }));
        }
      }
    }
  }
      ...
      for (const topic of cat.topics || []) {
        for (const quiz of topic.quizzes || []) {
          for (let i = 0; i < (quiz.questions || []).length; i++) {
            items.push({ filePath, quizId: quiz.id || quiz.title || `${topic.id || topic.title}`, question: quiz.questions[i], index: i + 1, style: 'new-exam' });
          }
        }
      }
    }
...
function isObjectOptionStyle(q) {
  return Array.isArray(q?.answerOptions) && q.answerOptions.length > 0 && typeof q.answerOptions[0] === 'object';
}
function isStringOptionStyle(q) {
  return Array.isArray(q?.answerOptions) && q.answerOptions.length > 0 && typeof q.answerOptions[0] === 'string';
}
...
function autoPickCorrect(q) {
  ...
  let idx = q.answerOptions.findIndex(o => typeof o?.rationale === 'string' && /\bcorrect\b/i.test(o.rationale));
  if (idx >= 0) return idx;
...
function validateQuestion(q, idx, issues, filePath, quizId) {
  ...
  if (hasOptions && isObjectOptionStyle(q)) {
    normalizeBooleans(q);
    const correctOptions = q.answerOptions.filter(opt => opt.isCorrect === true);
    if (correctOptions.length !== 1) {
      issues.push(`${context} Expected exactly 1 correct option, found ${correctOptions.length}`);
    }
    q.answerOptions.forEach((opt, oi) => {
      if (typeof opt.isCorrect !== 'boolean') {
        issues.push(`${context} opt ${oi + 1} isCorrect must be boolean true/false`);
      }
      if (!opt.rationale || typeof opt.rationale !== 'string' || !opt.rationale.trim()) {
  ...
  const multipleClaim = /(both\s+[A-D]\s+and\s+[A-D])|(also\s+(?:option\s+)?[A-D]\b)/.test(rat);
      if (multipleClaim) {
    ...
    if (!('correctAnswer' in q)) {
      issues.push(`${context} Multiple-choice string options but no correctAnswer field`);
    } else {
    ...
    if (!('correctAnswer' in q)) {
      issues.push(`${context} No multiple choice and no correctAnswer provided`);
    } else if (
...
try {
  const fracUtil = requireCJS(path.join(root, 'backend', 'utils', 'fractionPlainText.js'));
...
function fracToNumber(n, d) { return d === 0 ? NaN : n / d; }
function decimalToFraction(x, maxDen = 1000) {
  ...
  do {
    ...
    const h2 = a * h1 + h0; h0 = h1; h1 = h2;
    const k2 = a * k1 + k0; k0 = k1; k1 = k2;
    const frac = h1 / k1;
...
function normalizeMathText(text) {
  let t = String(text || '');
  try { t = replaceLatexFractionsWithSlash(t).value || t; } catch { /* noop */ }
  t = t.replace(/\$/g, '');
  return t;
}
...
function computeExpectedFromQuestion(text) {
  ...
  let m = t.match(/(?:decimal\s+equivalent\s+of\s+the\s+fraction|convert\s+(?:the\s+)?fraction)\s+(-?\d+)\s*\/\s*(\d+)/i);
  if (m) {
  ...
  m = t.match(/(?:convert|express)\s+(-?\d+(?:\.\d+)?)\s+(?:to|as)\s+(?:a\s+)?fraction/i);
  if (m) {
    const v = parseFloat(m[1]);
    const f = decimalToFraction(v);
  ...
  m = t.match(/(add|subtract|multiply|divide)\s+the\s+fractions\s*:?\s*(-?\d+)\s*\/\s*(\d+)\s*([+\-x\*÷\/])\s*(-?\d+)\s*\/\s*(\d+)/i);
  if (m) {
    const op = m[4];
    const a = parseInt(m[2], 10), b = parseInt(m[3], 10), c = parseInt(m[5], 10), d = parseInt(m[6], 10);
    const res = opMap[op] ? opMap[op](a, b, c, d) : null;
    if (res) { const s = simplifyFrac(res.n, res.d); return { kind: 'fraction', n: s.n, d: s.d }; }
  }
...
function maybeCheckMath(q, issues, filePath, quizId, idx) {
  ...
  if (isObjectOptionStyle(q)) {
    const correct = (q.answerOptions || []).find(o => o && o.isCorrect === true);
  ...
  if (!optionMatchesExpectedText(correctText, expected)) {
    const expectedStr = expected.kind === 'number'
      ? `${expected.value}`
      : `${expected.n}/${expected.d}`;
    issues.push(`${context} Math check: marked correct answer does not match computed result (expected ${expectedStr})`);
  }
}
...
function applyStructuralFixes(datasetTag, fixesSummary) {
  ...
  for (const { question: q, index, quizId } of items) {
    ...
    if (correctCount !== 1) {
      ...
      if (pick >= 0) {
        q.answerOptions.forEach((o, i) => { o.isCorrect = i === pick; });
      }
    }
    // Ensure rationale strings exist
    q.answerOptions.forEach((o) => {
      if (!o.rationale || typeof o.rationale !== 'string' || !o.rationale.trim()) {
        o.rationale = o.isCorrect ? 'Correct. This option best satisfies the question.' : 'This option does not satisfy the requirements of the question.';
      }
    });
    ...
    if (before !== after) {
      ...
      fixesSummary.fixes.push({
        ...
        issue: 'Structural normalization (isCorrect count/booleans and rationale placeholders)',
...
function writeFrontendQuizData(datasetTag) {
  ...
  const content = banner + JSON.stringify(datasetTag.data, null, 4) + ';\n' + trailer;
...
async function main() {
  ...
  for (const tag of datasets) {
    ...
    for (const it of questions) {
      ...
      try { maybeCheckMath(it.question, issues, it.filePath, it.quizId, it.index); } catch {/* noop */}
    }
  ...
  if (issues.length) {
    ...
    log(`All quizzes passed validation ✅ (checked ${totalQuestions} questions)`);
    process.exitCode = 0;
  }
}
