...
export function calculatePensionBenefit(careerState, monthsWorked, retirementConfig) {
...
export function initRetirementIncome(player, careerState, retirementConfig) {
  ...
  return {
    social_security: estimateSocialSecurity(careerState.current_wage, retirementConfig),
    pension: calculatePensionBenefit(careerState, player.monthsWorked || 0, retirementConfig),
...
export function computeRetirementMonthlyNet(retirementIncome, taxConfig) {
  const grossIncome = retirementIncome.social_security + 
  ...
  const taxableIncome = (retirementIncome.social_security * 0.85) + 
                         retirementIncome.pension + 
                         retirementIncome.withdrawal;
...
export function gradeRetirementOutcome(netWorth, monthlyIncome, monthlyExpenses, ageAtRetirement) {
