...
function ConstitutionExplorer({ onExit }) {
  ...
  const filtered = AMENDMENTS_DB.filter(item => 
      ...
      <div className="max-w-5xl mx-auto p-6 space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {filtered.map(amendment => (
            <div key={amendment.id} className="flex flex-col p-5 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 shadow-sm hover:shadow-md transition-shadow">
              ...
              <p className={`text-base leading-relaxed flex-grow ${mode === 'original' ? 'font-serif italic text-slate-700 dark:text-slate-300' : 'font-sans text-slate-900 dark:text-white'}`}>
                {mode === 'original' ? `"${amendment.original}"` : amendment.simple}
              </p>
            </div>
          ))}
          {filtered.length === 0 && (
