...
function loadBestScores(userId) {
  ...
  try {
    return JSON.parse(
      window.localStorage.getItem(bestScoresKey(userId)) || '{}'
    );
  } catch {
    return {};
  }
...
function ScenarioRunner({ scenario, userId, onExit }) {
  ...
  return (
    <div className="space-y-4 max-w-3xl">
      ...
      <div className="rounded-xl border bg-white dark:bg-slate- text-slate-900 dark:text-slate-100 p-4">
        ...
        {!isTerminal ? (
          <div className="mt-4 space-y-2">
            {node.choices.map((c, i) => (
              <button
                ...
                type="button"
                onClick={() => pick(c)}
                className="w-full text-left px-4 py-2 rounded-lg border hover:border-teal-400 hover:bg-teal-50 dark:hover:bg-teal-900/20"
              >
                {c.label}
              </button>
            ))}
          ...
          <div className="mt-4">
            <div className="rounded-lg p-3 bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-300">
              <div className="text-xs uppercase tracking-wide text-emerald-700 dark:text-emerald-300 font-bold">
                Outcome
              </div>
              <div className="text-base font-semibold mt-1">Score: {score}</div>
              {node.summary ? (
                <p className="text-sm mt-2">{node.summary}</p>
              ) : null}
            </div>
            <div className="mt-3 flex gap-2">
              ...
              <button
                type="button"
                onClick={onExit}
                className="px-4 py-2 rounded bg-slate-200 dark:bg-slate-700 text-sm font-semibold"
              >
                Pick another
              </button>
            </div>
            ...
            {transcript.map((t, i) => (
              <li key={i}>
                ...
                <div className="font-semibold">
                  → You chose: {t.choice} (+{t.score})
                </div>
                {t.feedback ? (
                  <div className="text-xs text-slate-500">{t.feedback}</div>
                ) : null}
              </li>
            ))}
          </ol>
...
export default function WorkplaceSkillsSims({ onBack, userId = 'anon' }) {
  ...
  return (
    <WorkforceSectionFrame
      ...
      subtitle="Branching scenarios on conflict, customers, ethics, and feedback. Choices have consequences."
      onBack={onBack}
    >
      <div className="px-4 pb-6">
        <div className="grid sm:grid-cols-2 gap-3">
          {SCENARIOS.map((s) => (
            <button
              ...
              className="text-left rounded-xl border bg-white dark:bg-slate- text-slate-900 dark:text-slate-100 p-4 hover:border-teal-400 hover:shadow-lg transition"
            >
              ...
              {bestScores[s.id] != null ? (
                <div className="text-xs text-emerald-700 dark:text-emerald-300 mt-2 font-semibold">
                  Best score: {bestScores[s.id]}
                </div>
              ) : (
