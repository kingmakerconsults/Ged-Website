...
import {
  dailyPromptForDate,
  SOFT_SKILLS_PROMPTS,
} from './data/softSkillsPrompts.js';
...
function saveJournal(userId, journal) {
  if (typeof window === 'undefined') return;
  try {
    window.localStorage.setItem(storageKey(userId), JSON.stringify(journal));
  } catch {}
}
...
function localCoachFeedback(entry, prompt) {
  ...
  const lines = [];
  if (wc < 30)
    lines.push('Try to push past 30 words. Specifics beat summaries.');
  if (!/I |my /i.test(entry || ''))
    lines.push(
      'Use "I" statements to anchor the reflection in your experience.'
    );
  if (!/(because|so|therefore|which|when)/i.test(entry || ''))
    lines.push('Add a "because" or "so that" - reasoning beats observation.');
  if (lines.length === 0)
    lines.push(
      'Solid reflection. Bonus: identify one tiny experiment you\u2019ll try this week.'
    );
...
export default function SoftSkillsCoach({ onBack, userId = 'anon', apiBase }) {
  ...
  const promptObj = useMemo(
    () => dailyPromptForDate(new Date(`${date}T12:00:00`)),
    [date]
  );
  ...
  async function getCoach() {
    ...
    try {
      const res = await fetch(
        `${apiBase || ''}/api/workforce/soft-skills-evaluate`,
        {
          method: 'POST',
  ...
  return (
    <WorkforceSectionFrame
      ...
      subtitle="Daily reflection prompts. Build the habit. Get coach feedback when you want it."
      onBack={onBack}
    >
      <div className="px-4 pb-6 grid md:grid-cols-3 gap-4">
        <main className="md:col-span-2 space-y-3">
          <div className="rounded-xl border border-teal-300 bg-teal-50 dark:bg-teal-900/20 p-4">
            <div className="flex items-center justify-between gap-2">
              ...
              <input
                ...
                className="px-2 py-1 rounded border bg-white dark:bg-slate- text-slate-900 dark:text-slate-100 text-sm"
              />
            </div>
          ...
          <div className="flex gap-2 items-center">
            <button
              type="button"
              onClick={getCoach}
              disabled={loading || !todayEntry.trim()}
        ...
        <aside className="md:col-span-1 space-y-3">
          <div className="rounded-xl border bg-white dark:bg-slate- text-slate-900 dark:text-slate-100 p-4 text-center">
          ...
          <div className="rounded-xl border bg-white dark:bg-slate- text-slate-900 dark:text-slate-100 p-4 text-center">
            ...
            <div className="text-xs text-slate-500">
              of {SOFT_SKILLS_PROMPTS.length} prompts available
            </div>
          </div>
          <div>
            ...
            {recentEntries.length === 0 ? (
              ...
              <ul className="space-y-2">
                {recentEntries.map(([d, v]) => (
                  <li
                    key={d}
                    className="rounded border p-2 bg-white dark:bg-slate- text-slate-900 dark:text-slate-100"
                  >
                    <button
                      type="button"
                      onClick={() => setDate(d)}
                    ...
                    >
                      ...
                      <div className="text-xs italic text-slate-500 line-clamp-1">
                        {v.prompt}
                      </div>
                      <div className="text-xs text-slate-700 dark:text-slate-300 line-clamp-2 mt-0.5">
                        {v.entry}
                      </div>
                    </button>
                  </li>
