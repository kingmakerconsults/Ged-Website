...
function CareerCard({ career, onOpen }) {
  return (
    <button
      ...
      className="text-left rounded-xl border bg-white dark:bg-slate- text-slate-900 dark:text-slate-100 p-4 hover:shadow-lg hover:border-teal-400 transition"
    >
      ...
      <div className="flex items-center justify-between mt-3 text-xs">
        ...
        <span className="text-slate-500">
          {EDUCATION_LEVELS.find((e) => e.key === career.education)?.label ||
            career.education}
        </span>
      </div>
    </button>
...
function CareerModal({ career, onClose }) {
  ...
  return (
    ...
    >
      <div
        className="bg-white dark:bg-slate- text-slate-900 dark:text-slate-100 rounded-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto p-6 shadow-2xl"
      ...
      >
        <div className="flex items-start justify-between gap-3 mb-3">
          <div>
            ...
            <div className="text-sm font-mono mt-1">
              {fmtMoney(career.medianWage)} median -{' '}
              {EDUCATION_LEVELS.find((e) => e.key === career.education)?.label}
            </div>
          </div>
          <button
        ...
        <section className="mb-3">
          <h4 className="font-bold text-sm uppercase tracking-wide text-slate-500">
            A day in the life
          </h4>
        ...
        {career.relatedModules?.length > 0 ? (
          <section>
            <h4 className="font-bold text-sm uppercase tracking-wide text-slate-500">
              Related Core Digital Skills modules
            </h4>
...
export default function CareerPathwaysExplorer({ onBack }) {
  ...
  return (
    <WorkforceSectionFrame
      ...
      subtitle="Filter careers by interest, education, and wage. Click for day-in-the-life details."
      onBack={onBack}
    >
      <div className="px-4 pb-6 grid md:grid-cols-4 gap-4">
        <aside className="md:col-span-1 space-y-3">
          ...
          <div>
            <label className="block text-xs font-bold uppercase tracking-wide text-slate-500">
              Min median wage: {fmtMoney(minWage)}
            </label>
            <input
              type="range"
              min={0}
          ...
          <div>
            <label className="block text-xs font-bold uppercase tracking-wide text-slate-500">
              Max median wage: {fmtMoney(maxWage)}
            </label>
            <input
              type="range"
              min={20000}
        ...
        <main className="md:col-span-3">
          {filtered.length === 0 ? (
            <div className="text-sm italic text-slate-500 p-4 border border-dashed rounded">
              No careers match. Try widening filters.
            </div>
          ) : (
            <div className="grid sm:grid-cols-2 gap-3">
              {filtered.map((c) => (
                <CareerCard key={c.id} career={c} onOpen={setActive} />
              ))}
            </div>
          )}
        </main>
