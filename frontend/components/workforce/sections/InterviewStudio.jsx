    ...
    <div className="space-y-3">
      <div className="flex flex-wrap items-center gap-2">
        ...
        <input
          ...
          className="flex-1 min-w-[180px] px-2 py-1 rounded border bg-white dark:bg-slate- text-slate-900 dark:text-slate-100"
        />
        <span className="text-xs text-slate-500">
          Practiced: {practicedCount}/{INTERVIEW_QUESTIONS.length}
        </span>
      </div>
      <ul className="space-y-2">
        {filtered.map((q) => (
          <li
            key={q.id}
            className="flex items-start gap-3 p-3 rounded border bg-white dark:bg-slate- text-slate-900 dark:text-slate-100"
          >
            <input
              type="checkbox"
              checked={!!practiced[q.id]}
              onChange={(e) =>
                setPracticed((p) => ({ ...p, [q.id]: e.target.checked }))
              }
              className="mt-1"
            />
            <div className="flex-1">
              ...
              {q.tip ? (
                <div className="text-xs italic text-slate-600 dark:text-slate-400 mt-1">
                  Tip: {q.tip}
                </div>
              ) : null}
            </div>
        ...
        {filtered.length === 0 ? (
          <li className="text-sm italic text-slate-500">No questions match.</li>
        ) : null}
      </ul>
    </div>
  );
...
function localStarReview(star) {
  ...
  if (wc(star.situation) < 15)
    lines.push('- Situation: add concrete context (when, where, who).');
  if (wc(star.task) < 10)
    lines.push(
      "- Task: clarify YOUR specific responsibility (not the team's)."
    );
  if (wc(star.action) < 25)
    lines.push(
      '- Action: this should be the longest part. Show the steps you personally took.'
    );
  if (wc(star.result) < 10)
    lines.push(
      '- Result: quantify if you can (% improvement, $, time saved, customers retained).'
    );
  if (!/I |my /i.test(star.action))
    lines.push(
      '- Use "I" not "we" in Action - interviewers want YOUR contribution.'
    );
  if (lines.length === 0)
    lines.push(
      'Looks balanced. Practice saying it out loud - aim for 60-90 seconds.'
    );
  ...
  return (
    <div className="space-y-3 max-w-3xl">
      <p className="text-sm text-slate-600 dark:text-slate-400">
        STAR = Situation, Task, Action, Result. Build a 60-90 second story you
        can adapt to many behavioral questions.
      </p>
      <input
        value={star.question}
        onChange={(e) => setStar((s) => ({ ...s, question: e.target.value }))}
        placeholder="The question (e.g., Tell me about a time you handled conflict)"
      ...
      {['situation', 'task', 'action', 'result'].map((field) => (
        <div key={field}>
          ...
          <textarea
            ...
            value={star[field]}
            onChange={(e) =>
              setStar((s) => ({ ...s, [field]: e.target.value }))
            }
            className="w-full px-2 py-1 rounded border bg-white dark:bg-slate- text-slate-900 dark:text-slate-100"
          />
        </div>
      ...
      <button
        ...
        onClick={getReview}
        disabled={loading}
        className="px-4 py-2 rounded bg-teal-600 text-white text-sm font-semibold disabled:opacity-50"
      >
        {loading ? 'Reviewing...' : 'Get coach feedback'}
      </button>
      {error ? (
        <div className="text-xs text-amber-600">
          Online coach unavailable; showing local review. ({error})
        </div>
      ) : null}
      {feedback ? (
...
function AIMock({ userId, apiBase }) {
  ...
  async function send() {
    ...
    try {
      ...
      setHistory((h) => [
        ...h,
        { role: 'interviewer', text: localMockReply(newHistory) },
      ]);
    } finally {
      setLoading(false);
    }
      ...
      <div className="space-y-2 max-h-96 overflow-y-auto p-3 rounded border bg-slate-50 dark:bg-slate-900">
        {history.length === 0 ? (
          ...
          history.map((m, i) => (
            ...
            >
              <div
                className={`inline-block px-3 py-2 rounded-lg max-w-[85%] ${m.role === 'user' ? 'bg-teal-600 text-white' : 'bg-white dark:bg-slate- text-slate-900 dark:text-slate-100 border'}`}
              >
      ...
      {!done ? (
        <div className="flex gap-2">
          <textarea
            ...
            className="flex-1 px-2 py-1 rounded border bg-white dark:bg-slate- text-slate-900 dark:text-slate-100"
            onKeyDown={(e) => {
          ...
          <button
            ...
            className="px-4 py-2 rounded bg-teal-600 text-white text-sm font-semibold disabled:opacity-50 self-end"
          >
            {loading ? '...' : 'Send'}
          </button>
        </div>
        ...
        <div className="rounded-xl border bg-emerald-50 dark:bg-emerald-900/20 p-4 text-sm">
          Mock interview complete. Review the transcript above. Reset to try
          again.
        </div>
      )}
      {error ? (
...
function SelfRecord() {
  ...
  async function start() {
    ...
    if (
      ...
      setError(
        'Your browser does not support recording. Try Chrome, Edge, or Firefox on desktop.'
      );
      return;
    }
    try {
      ...
      setError(
        e.message || 'Could not access camera/microphone. Check permissions.'
      );
    }
  }
  ...
  return (
    <div className="space-y-3 max-w-2xl">
      <p className="text-sm text-slate-600 dark:text-slate-400">
        Record yourself answering one of the bank questions. Watch yourself
        back. Look for: filler words, eye contact, pace, energy.
      </p>
      {error ? (
        <div className="text-sm rounded p-3 bg-amber-50 dark:bg-amber-900/20 text-amber-800 dark:text-amber-200">
          {error}
        </div>
      ) : null}
      <div className="flex gap-2">
      ...
      {blobUrl ? (
        <div className="space-y-2">
          ...
          <a
            href={blobUrl}
            download={`practice-${Date.now()}.webm`}
      ...
      <p className="text-xs text-slate-500 italic">
        Recordings stay in your browser only. They are not uploaded.
      </p>
    </div>
  );
}
...
export default function InterviewStudio({ onBack, userId = 'anon', apiBase }) {
  ...
  return (
    <WorkforceSectionFrame
      ...
      subtitle="Question bank, STAR builder, AI mock interview, and self-recording."
      onBack={onBack}
    >
