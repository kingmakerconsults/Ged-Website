...
quizFiles.forEach(file => {
  ...
  const quizzes = Array.isArray(data) ? data : (data.quizzes ? Object.values(data.quizzes).flat() : []);
...
if (brokenReferences.length > 0) {
  ...
  if (brokenReferences.length > 30) {
    console.log(`... and ${brokenReferences.length - 30} more broken references\n`);
  }
}
...
fs.writeFileSync('image-audit-detailed-report.json', JSON.stringify(report, null, 2));
