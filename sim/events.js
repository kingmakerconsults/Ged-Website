...
export function rollMonthlyEvents(player, eventsCfg, eventLog, currentMonth, currentYear) {
  let triggered = [];
  ...
  for (const event of eventsCfg) {
    ...
    if (event.recurs_every_years) {
      ...
      if (lastMonth && (currentMonth - lastMonth) < event.recurs_every_years * 12) {
        continue;
      }
    }
...
function applyEventEffects(player, event) {
  ...
  if (event.monthly_rent_delta_pct && player.dwelling) {
    player.rentMultiplier = (player.rentMultiplier || 1) * (1 + event.monthly_rent_delta_pct);
  }
  ...
  if (eff.investment_loss_pct) {
    player.investments = (player.investments || 0) * (1 + eff.investment_loss_pct);
  }
